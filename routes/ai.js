...
TENSE MATTERS: if the open thread is something PLANNED FOR THE FUTURE relative to today (a next session, an upcoming call, "I'll deal with it tomorrow", "meeting them on Monday"), the question must stay forward-looking — how they're feeling about it, what they're expecting, anything on their mind about it. NEVER phrase it as if the event already happened ("How did it go?", "How was it?") when the entry itself describes it as still ahead — that reads as asking the user to predict the future.`;

...
const { postBugReport, APP_NAME } = require('./orbit');

const AUTO_SOURCES = new Set(['frontend', 'backend']);

// Forward one stored report to Orbit's dedicated bug/idea intake; flip
// orbit_sent on success. Ideas & Bugs checklist entries carry
// context.type = 'idea'|'bug' (see settings.js); everything else (auto-
// captured frontend/backend errors) is a bug_report by nature.
async function forwardReport(r) {
  const res = await postBugReport({
    type: r.context?.type === 'idea' ? 'idea' : 'bug_report',
    message: r.message,
    stack: r.stack,
    context: r.context,
  ...
  if (res.ok) {
    // orbit_id is required to later resolve/delete this report on Orbit's
    // side — resolveBugReport matches by id only, no content fallback.
    const orbitId = res.body?.id || null;
    await db.query('UPDATE reports SET orbit_sent = TRUE, orbit_id = $2 WHERE id = $1', [r.id, orbitId]).catch(() => {});
  }
  return res;
}
...
async function flushUnsent() {
  // Retention: drop reports older than 60 days so the table can't grow forever.
  try {
    await db.query("DELETE FROM reports WHERE created_at < NOW() - INTERVAL '60 days'");
  } catch { /* best-effort */ }

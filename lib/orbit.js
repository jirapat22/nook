...
// Bug/idea reports — a distinct endpoint from graph-node ingest. Posting a
// report through /api/ingest with target: 'Feedback' 400'd ("Category
// 'Feedback' not found") because that's not a real graph category; Orbit's
// actual feedback intake is /api/bug-reports with its own payload shape.
async function postBugReport({ type, message, stack, context, created_at }) {
  return postOrbitEndpoint('/api/bug-reports', {
    app: APP_NAME,
...
module.exports = { postToOrbit, postBugReport, syncPerson, markPersonDeleted, syncAllPeople, APP_NAME };

...
// Orbit endpoints authenticated with X-API-Key rather than x-ingest-secret.
// Two different auth schemes exist because Orbit's bug-reports API predates
// the ingest-secret convention and was never unified with it; /api/ingest/nodes
// follows the same X-API-Key scheme, so both share this helper.
// (Posting a report through /api/ingest with target: 'Feedback' 400'd —
// "Category 'Feedback' not found" — because that's not a real graph category.)
async function orbitKeyedRequest(method, path, payload) {
  const url = process.env.ORBIT_URL;
  const secret = process.env.ORBIT_INGEST_SECRET;
  if (!url || !secret) return { skipped: true, reason: 'ORBIT_URL or ORBIT_INGEST_SECRET not configured' };
  try {
    ...
    if (!res.ok) {
      console.warn(`[Orbit] ${method} ${path} non-ok ${res.status}:`, JSON.stringify(body).slice(0, 200));
      return { ok: false, status: res.status, body };
    }
    return { ok: true, body };
  } catch (err) {
    console.warn(`[Orbit] ${method} ${path} failed:`, err.message);
    return { ok: false, error: err.message };
  }
}
...
async function postBugReport({ type, message, stack, context, created_at }) {
  return orbitKeyedRequest('POST', '/api/bug-reports', {
    app: APP_NAME,
...
async function resolveBugReport(orbitId) {
  ...
  return orbitKeyedRequest('PATCH', `/api/bug-reports/${orbitId}`, { status: 'resolved' });
}

// Read back the nodes Orbit currently holds for this app.
// Returns { ok, nodes: [{ id, name }] } — `id` is ORBIT's node id, not the
// externalId Nook sent, and group nodes are already filtered out server-side.
async function fetchOrbitNodes() {
  const res = await orbitKeyedRequest('GET', `/api/ingest/nodes?source=${APP_NAME}`);
  if (!res.ok) return res;
  return { ok: true, nodes: Array.isArray(res.body?.nodes) ? res.body.nodes : [] };
}

const normName = s => String(s ?? '').trim().toLowerCase();

// Compare what Orbit holds against what Nook actually has.
//
// Nook keeps no tombstones, so it cannot enumerate people it has already
// deleted — which is how a node for a deleted/merged person can sit on the
// graph forever with nothing on either side able to notice (exactly what
// happened with a stale "Emily" node from 13 Jul 2026). Reading Orbit's list
// back and diffing it is the only way to see those.
//
// Matched by NAME, because Orbit's list returns its own node ids rather than
// the externalId Nook sent — name is the only join key available. That makes
// this a report, not an automatic cleanup: a name collision or an odd bit of
// whitespace should never be enough to archive someone's contact on its own.
// Aliases count as a match too, so renaming someone to a name they already had
// as an alias doesn't make their live node look stale.
async function reconcileOrbitNodes() {
  const remote = await fetchOrbitNodes();
  if (!remote.ok) return remote;

  let people;
  try {
    const r = await db.query('SELECT id, name, aliases FROM people');
    people = r.rows;
  } catch (err) {
    return { ok: false, error: err.message };
  }
  ...
  for (const p of people) {
    ...
    for (const a of (Array.isArray(p.aliases) ? p.aliases : [])) {
      if (a) known.add(normName(a));
    }
  }

  // On Orbit but not in Nook — stale nodes to delete over there.
  const stale = remote.nodes
    .filter(n => n?.name && !known.has(normName(n.name)))
    .map(n => ({ id: n.id, name: n.name }));

  // In Nook but not on Orbit — a push that never landed. Should stay empty now
  // that flushUnsyncedPeople retries; if it isn't, the sync is still failing.
  const remoteNames = new Set(remote.nodes.map(n => normName(n?.name)));
  const missing = people
    .filter(p => p.name && !remoteNames.has(normName(p.name)))
  ...
  return {
    ...
    stale,
    missing,
    counts: { orbit: remote.nodes.length, nook: people.length },
  };
}
...
module.exports = {
  ...
  fetchOrbitNodes, reconcileOrbitNodes,

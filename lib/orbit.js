...
async function bugReportsRequest(method, path, payload) {
  ...
  try {
    ...
    console.log(`[Orbit] bug-reports ${method} ${path} ok:`, JSON.stringify(body).slice(0, 200));
    return { ok: true, body };
  } catch (err) {
...
module.exports = { postToOrbit, postBugReport, resolveBugReport, syncPerson, markPersonDeleted, syncAllPeople, APP_NAME };

...
async function personToOrbitItem(person, mentions) {
  ...
  // Only send when we have a real value — omitting it leaves the person
  // directly under People, which is the desired "uncategorised" behavior.
  if (person.relationship_type) {
    item.relationship_type = person.relationship_type;
  }
  // Friend circles / subgroups — Orbit can use this for a deeper nesting level
  // (People → Friend → Uni gang → Alice).
  if (person.subgroup) {
    item.subgroup = person.subgroup;
  }
  // "Met through" — edge to another node. Use the OTHER person's externalId
  // (same format we use for them) so Orbit can resolve the link.
  if (person.introduced_by_id) {
    item.introduced_by_external_id = `nook-person-${person.introduced_by_id}`;
  }
  return item;
}
